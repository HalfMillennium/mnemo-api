...
from mnemo_api.mnemo_logic.search.page_parser.page_parser_service import PageParserService
from mnemo_api.mnemo_logic.gpt_api.service import GptService

class MnemoService:
    '''
    Return AI generated first-person POV diary entry based on the current events related to the search term entity_name
    '''
    async def fetch_diary_entry(self, entity_name):
        ...
        page_html = await google_news_search.execute_query(entity_name)
        ...
        diary_entry_response = await gpt_service.generate_diary_entry(parsed_stories, entity_name)
        return diary_entry_response

    '''
    Returns a set of relevant images related to the search term entity_name
    '''
    async def fetch_entity_images(self, entity_name):
        yahoo_images_search = SearchResourceService("Yahoo Images")
        images_list = list(await yahoo_images_search.fetch_and_parse_images(entity_name))
        return images_list
    
    '''
    Returns a summary of the search term entity_name
    '''
    def fetch_entity_summary(self, entity_name):
        gpt_service = GptService()
